...
import { PlusOutlined, EditOutlined, DeleteOutlined, CloudUploadOutlined, EyeOutlined } from '@ant-design/icons';
import { secretApi, namespaceApi } from '@/services/api';
import type { Secret, Namespace } from '@/types';

export default function SecretList() {
  const [tableData, setTableData] = useState<Secret[]>([]);
  const [loading, setLoading] = useState(false);
  const [modalVisible, setModalVisible] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Secret | null>(null);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const data = await secretApi.list({ page: 1, page_size: 100 });
  ...
  const handleAdd = async (values: any) => {
    try {
      ...
      if (editingRecord) {
        await secretApi.update(editingRecord.id, values);
  ...
  const handleEdit = (record: Secret) => {
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            title="确认删除? 已发布的Secret将同时从K8s中删除"
  ...
  return (
    <div>
      ...
      <Modal
        title={editingRecord ? '编辑保密字典' : '创建保密字典'}
        open={modalVisible}
        onCancel={() => {
          setModalVisible(false);
          form.resetFields();
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleAdd}>
          ...
          >
            <Input placeholder="请输入标识，如: db-credentials" />
          </Form.Item>
          <Form.Item
            name="namespace_id"
            label="命名空间"
          ...
          <Card
            title="保密数据"
            extra={<Button type="link" onClick={addDataRow}>添加密文项</Button>}
          ...
          >
            {dataRows.map((_, index) => (
              <div key={index} style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                ...
                >
                  <Input.Password placeholder="敏感数据" />
                </Form.Item>
                {dataRows.length > 1 && (
                  <Button
                    danger
